...
// ===== Sidecar subtitle helpers (shared shape with viewerSerie.js) =====
...
async function domInserter() {
  ...
  for (const s of trackData.subtitles || []) {
    ...
    trackHtml += `<track kind="subtitles" src="/api/subtitle?path=data/movies/${encodeURIComponent(id)}.mp4&index=${s.index}"${lang} label="${label}" data-key="embedded:${s.index}">`;
  }
  ...
  // subtitles are added dynamically once fetched, see initSidecarSubtitles)
