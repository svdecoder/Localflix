...
function domInserter(dataArray, seasonNumber, titleOfSerie) {
  ...
  for (let i = 0; i < dataArray.length; i++) {
    ...
    let thumbnail = `/data/thumbnail/${escapeAttr(titleOfSerie)}/${escapeAttr(identifier)}.jpg`
    cartToInsert += `
      <a href="/viewerS?id=${encodeURIComponent(identifier)}" class="video-link">
      ...
      <span class="information">Episode number ${escapeHtml(String(episode))} from season ${escapeHtml(String(seasonNumber))}</span>
    </div>
  </a>`
  }
